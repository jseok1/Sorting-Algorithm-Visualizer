...
def bubble_sort(screen, lst):
    ...
    while k > 0:
        for i in range(k):
            ...
            Visualizer.update(screen, lst[:], {i, i + 1, k})
        k -= 1
    Visualizer.update(screen, lst[:], {})
    ...
    for i in range(len(lst)):
        ...
        for j in range(i, len(lst)):
            if lst[j] < lst[smallest]:
                smallest = j
            Visualizer.update(screen, lst[:], {i, j, smallest})
        lst[smallest], lst[i] = lst[i], lst[smallest]
        Visualizer.update(screen, lst[:], {i, smallest})
    Visualizer.update(screen, lst[:], {})
...
def insertion_sort(screen, lst):
    ...
    for i in range(0, len(lst)):
        ...
        while j > 0 and lst[j - 1] > key:
            ...
            Visualizer.update(screen, lst[:], {i, j - 1, j})
            j -= 1
        lst[j], key = key, lst[j]
        Visualizer.update(screen, lst[:], {i, j})
    Visualizer.update(screen, lst[:], {})
...
def _merge(screen, lst, low, mid, high):
    ...
    while i < mid + 1:
        merged.append(lst[i])
        Visualizer.update(screen, lst[:], {i})
        i += 1
    while j < high + 1:
        merged.append(lst[j])
        Visualizer.update(screen, lst[:], {j})
        j += 1
    n = 0
    for k in range(low, high + 1):
        lst[k] = merged[n]
        Visualizer.update(screen, lst[:], {k})
        n += 1
...
def _partition(screen, lst, low, high):
    ...
    while i < j:
        while lst[i] < pivot:
            Visualizer.update(screen, lst[:], {i, j, low})
            i += 1
        while lst[j] > pivot:
            Visualizer.update(screen, lst[:], {i, j, low})
            j -= 1
...
def heap_sort(screen, lst):
    ...
    for i in range(len(lst) - 1, 0, -1):
        ...
        Visualizer.update(screen, lst[:], {0, i})
...
def _heapify(screen, lst, size, i):
    ...
    left = 2 * i + 1
    right = 2 * i + 2
    Visualizer.update(screen, lst[:], {size, largest})
    if left < size:
        if lst[largest] < lst[left]:
            largest = left
        Visualizer.update(screen, lst[:], {size, largest, left})
    if right < size:
        if lst[largest] < lst[right]:
            largest = right
        Visualizer.update(screen, lst[:], {size, largest, right})
    if largest != i:
        lst[i], lst[largest] = lst[largest], lst[i]
        Visualizer.update(screen, lst[:], {size, largest, i})
